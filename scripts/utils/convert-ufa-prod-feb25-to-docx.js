cusconst fs = require('fs');
const path = require('path');
const { Document, Packer, Paragraph, TextRun, Table, TableRow, TableCell, HeadingLevel, AlignmentType } = require('docx');

const markdownFile = path.join(__dirname, '..', '..', 'deployments', 'UFA', 'UFA_Prod_Deployment_Feb_25_2026_Release_Notes.md');
const outputFile = path.join(__dirname, '..', '..', 'deployments', 'UFA', 'UFA_Prod_Deployment_Feb_25_2026_Release_Notes.docx');
...
Packer.toBuffer(doc).then(buffer => {
  ...
  console.log(`✓ UFA Production Release notes converted to DOCX`);
