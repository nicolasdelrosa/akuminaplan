  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('```')) {
      ...
      if (tableLines.length > 0) {
        sections.push(createTable(tableLines));
        sections.push(new Paragraph({ text: '', spacing: { after: 300 } }));
        i = j - 1;
      }
...
function printUsageAndExit() {
  console.error('Usage: node .github/skills/create-runbook/scripts/convert-runbook.js <input.md> [output.docx]');
  process.exit(1);
}
...
if (require.main === module) {
  ...
  if (inputFile === outputFile) {
    console.error('Input and output files must be different.');
    process.exit(1);
  }
  ...
  if (!/\.md$/i.test(inputFile)) {
    console.error('Input file must be a markdown file.');
    process.exit(1);
  }

  convertMarkdownToDocx(inputFile, outputFile)
    .then(() => {
      console.log(`Generated DOCX: ${outputFile}`);
    })
    .catch((error) => {
